...
Page({
	...
	data: {
		...
		IMG_URL:'',
		PAGE_PATH:'',
		jumpUrl:'https://vbank.jsbchina.cn/wbank/wxapp/tzApp.do?loginType=0&fnType=1&fnCode=',
		lxfsList:[
			{
				"branch":"无锡分行",
				"contacts":"费经理",
	...
	apply: function(e) {
		wx.navigateTo({
		  	url: '/sub3/pages/bbx/talent_finance_apply',
		})
	},
	...
	async getFin(){
		let res;
		let {preffixUrl,IMG_URL,PAGE_PATH} = this.data;
		try{
			res = await user.getTalentFin();
		} catch (err) {
			console.log(err)
		}
		this.setData({
			IMG_URL: preffixUrl + '/static/wechat/img/rcfunds/' +res.IMG_URL,
	...
	getStaff: async function(bbx_channel){
		...
		empInfo = {
			avatar: staff.HEAD || preffixUrl+'/static/wechat/img/bbx/avatar_default.png',
	...
	jumpToUrl:function(e){
		let { jumpUrl,PAGE_PATH } = this.data;
		let url = jumpUrl + PAGE_PATH ;
		console.log(url)
		wx.navigateTo({
