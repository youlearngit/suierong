...
Page({
	...
	onRegionCancel: function (e) {
		...
		this.setData({
			['filter.items.'+type+'.value'] : {value:[],code:[]},
		});
	},
	...
	onTreeSelectEvent: function (e) {
		let {filter_def,tree_select} = this.data;
	...
	onFilterEvent: async function (e) {
		let {location,bbx_channel} = this.data;
		switch (e.target.dataset.event) {
			case 'get_filter':{
				this.setData({
					'filter.show' : true,
				...
				if (!this.data.filter.items) {
					...
					if (bbx_channel=='320282') { // 宜兴
						delete filter_items.areas;
						delete filter_items.policy_levels;
					}
	...
	filter: function(filters) {
		let {type_to_tag,location_json} = this.data;
		let {doc_date,policy_levels,policy_types,areas} = filters;
		let doc_date1='', doc_date2='', area='', level='', type='', tag='';
		if (doc_date) {
		...
		if (areas) {
			...
			if (areas.type == 'picker_region') {
				area = areas.value.code.slice(-1)[0] || '';
			}
		...
		let url = `/sub3/pages/bbx/articles?search_type=filter&doc_date1=${doc_date1}&doc_date2=${doc_date2}&area=${area}&level=${level}&type=${type}&tag=${tag}&location=${location_json}`;
	...
	async initData(options) {
		...
		let {types,sub_types,type_to_tag,filter_def,bbx_channel} = this.data;
        ...
        this.setData({ location, location_json, types, sub_types, type_to_tag, filter_def, bbx_channel })
        this.searchArticles();
    },
