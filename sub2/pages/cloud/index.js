...
Page({
	...
	toCCY(phoneNumber) {
		...
		http
			...
			.then(res => {
				...
				let url = MYURLS.Urls.zmyPlatformYdt;
				wx.navigateTo({
		...
		if (phoneNumber != "") {
			wx.showModal({
				...
				success(res) {
					...
					if (res.cancel) {
						wx.navigateTo({
							url: "ydt?type=" + that.data.type + "&type2=1", //0是XIUGAI
						});
					}
				},
			});
			...
			wx.navigateTo({
				url: "ydt?type=" + that.data.type + "&type2=1", //1是添加
			});
		}
	},
	...
	toApply() {
		wx.navigateTo({
			url: "apply?page=1", //0是从跨境申请  1是自贸云申请
		});
	},
	...
	onLoad: function (options) {
		let that = this;
		this.setData({
			imgPreffixUrl: app.globalData.URL,
	...
	onShareAppMessage: function () {
		...
		var title = "自贸云平台";
		var path = "/sub2/pages/cloud/index";
		...
		if (phoneNumber != "") {
			wx.showModal({
				...
				success(res) {
					if (res.confirm) {
						wx.navigateTo({
							url: "cloudSearch?phoneNumber=" + phoneNumber,
						});
					}
					if (res.cancel) {
						wx.navigateTo({
