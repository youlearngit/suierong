...
Page({
  ...
  getData() {
    ...
    let promise1 = new Promise(function (resolve, reject) {
      ...
      wx.request({
        url: app.globalData.YTURL + 'jsyh/query2104.do',
        ...
        success(res) {
          if (res.data.head.H_STATUS === "1") {
            ...
            that.setData({
              idCardS: jsonData.ID_NO
            })
            if (jsonData.F02141_LIST != undefined) {
              jsonData.F02141_LIST.forEach((item, index) => {
                item.CARD_NOSub = item.CARD_NO.substring(item.CARD_NO.length - 4, item.CARD_NO.length)
              });
              resolve(jsonData.F02141_LIST)
              return;
            }
            resolve([])
          } else {
