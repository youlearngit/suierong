...
Page({
    ...
    getSignCode(entType,entCode){
      return new Promise((resolve, reject) => {
        let data = {
          signCodeTypeID: '3013979', //中征码查询身份标识  3013979,境内企业、3013980,境外企业、3013981,自然人
          ...
          // operateUserId: that.data.mangerInfo.USERIDLIST[0].USERID, //todo 业务经理id
    ...
    onLoad(e) {
        ...
        that.getBorrowInfo(e.business_id).then(res=>{
          that.getorgcode().then(res=>{
            if(e.business_type == '1' && e.companyInfo){
              var companyInfo = JSON.parse(e.companyInfo);
              if(companyInfo.CREDITCODE){
               that.verifySignCode(companyInfo);
              }
            }
          });
        })
    ...
    // 中征码校验
    verifySignCode(info){
        that.getSignCode('1',info.CREDITCODE).then(res=>{
          debugger
          if(res.status == 'success'){
              //获取中征码成功返现工商信息
                  that.setData({
                      credit_code: info.CREDITCODE,
                      'borrowerInfo.ENTERPRISE_NAME': info.ENTNAME,
                      'borrowerInfo.CREDIT_CODE': info.CREDITCODE,
                      'borrowerInfo.REGISTRATION_NO': info.REGNO,
                      'borrowerInfo.ENTERPRISE_CODE': info.ORGCODES,
                      'borrowerInfo.signCode': res.signCode
                  })
          }else{
            //获取中征码,组织机构代码
            that.getSignCode('2',info.ORGCODES).then(res=>{
              if(res.status == 'success'){
                  //获取中征码成功返现工商信息
                      that.setData({
                          credit_code: info.CREDITCODE,
