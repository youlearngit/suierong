...
function setBBXChannel(track_channel) {
	let bbx_channel = { // 通用版小程序 (默认)
		...
		name: '掌上三农', // 渠道名称 (后端接口调用) (eg. 掌上三农 | 宜兴人才百宝箱)
...
function shareWithBBXChannel({imgPath, params, title, url}) {
	...
	title = title || this.getBBXChannel().name || '掌上三农';
	url = url || '';
...
function preffixUrl() {
  ...
  return app.globalData.CDNURL +'/static/wechat/img/bbx/';
...
async function getRegionCode2(regionCode) {
	...
	let adCodeList = await api.getDistarct('320000', '3');
