...
export default class Talent {
	...
	static policyDisplay(policy) {
		...
		policy.AREA_text = policy.AREANAME || policy.APPLY_CITY; // 适用地区
		...
		policy.POLICYLEVEL_text = utils.policyFields().policy_levels[policy.POLICY_LEVEL]; // 政策层级
		...
		policy.POLICYCONTENT_html = utils.textToHtml(policy.POLICY_CONTENT); // 支持对象（政策内容）
  ...
  static async saveUsInfo(data) {
		let options = {
			url: 'zssn/saveUsInfo.do',
			data
		};
		let res = await requestYT(options);
  ...
  static async getMarketData(data) {
		let options = {
			url: 'zssn/getMarketData.do',
			data
		};
		let res = await requestYT(options);
		console.log(res);
  ...
  static async getAllMarketData(data) {
		let options = {
			url: 'zssn/getAllMarketData.do',
			data
		};
		let res = await requestYT(options);
		console.log(res);
  ...
  static async selectPriceIndex(data) {
		let options = {
			url: 'zssn/selectCategory.do',
			data
		};
		let res = await requestYT(options);
		console.log(res);
  ...
  static async projectList(data) {
		let options = {
			url: 'treasure/projectList.do',
			data
		};
		let res = await requestYT(options);
		console.log(res);
	...
	static async selectStaff(data) {	
		...
		let options = {
			url: 'talent/selectStaffMessage.do',
			data
		};
		let res = await requestYT(options);
		console.log(res);
	...
	static async getPolicy(data) {
		let options = {
			url: 'snzc/getPolicy.do',
			data
    };
    console.log(data);
		let res = await requestYT(options); 
	...
	static async selectPolicy(data) {
		let options = {
			url: 'need/queryPolicy.do',
			data
		};
		let res = await requestYT(options);
		if (res.STATUS === '1') {
...
// snzc/getPolicyList.do
static async getPolicyList(data) {
  let options = {
    url: 'snzc/getPolicyList.do',
    data
  };
  let res = await requestYT(options);
