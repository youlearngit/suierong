...
Page({
  ...
  onLoad: function (options) {
    ...
    toApplyLoanPage() {
        if (this.data.showNon2 == true &&
            ...
            //基础信息都有，则跳转到个人信息页面
            wx.navigateTo({
                url: '/sub3/pages/apply/loan/index?scene=' + this.data.scene,
            })
        } else {
            wx.showToast({
                title: '请补充身份信息',
  ...
  set_2: function () {
    ...
    if (!this.data.showNon1) {
      ...
      wx.navigateTo({
        url:
          '/sub3/pages/apply/orc/set_2?url=' +
          this.data.url +
          '&type=' +
          this.data.type +
          '&name=' +
  ...
  identifyDetail: function () {
    wx.navigateTo({
      url: '/sub3/pages/apply/orc/set_2_show',
    });
  },
