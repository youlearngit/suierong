...
Page({
  ...
  data: {
    ...
    avatar: '', //授权头像
    wechat: "", //本详情页二维码
    bg: "/static/wechat/img/temp/share_shui_bg2.jpg", //图片背景图
  ...
  onLoad: function(options) {
    //console.log(app.globalData.int_id + "&" + util.formatTime(new Date()))
    var ss = app.globalData.int_id + "&" + util.formatTime(new Date())
    ss.split("&")
    //console.log(ss)
    let userId = ss.split("&")[1]
    //console.log(userId)
    //openid，加密相关
  ...
  onShareAppMessage: function() {
    ...
    var path = 'pages/shui/index' + "?open_id=" + share_id + "&share_date=" + util.formatTime(new Date());
    //console.log(path)
    return {
      path: path,
      imageUrl: this.data.preffixUrl + "/static/wechat/img/temp/share_shui_f.jpg"
    }
  },
  prePage() {
    wx.navigateBack();
  ...
  showCreat: function(e) {
    ...
    if (that.data.submit) {
      ...
      //请求接口
      api.generateMiniCode("pages/shui/index").then(res=>{
        that.setData({
              ...
              if (err === 'unSelectIdcard') {
                wx.showModal({
                  ...
                  success: function (res) {
                    ...
                    wx.navigateTo({
                        url: '/sub1/pages/info/identify?type=3&url=/pages/shui/apli',
                    });
                  },
                });
              }
