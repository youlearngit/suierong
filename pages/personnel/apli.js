...
Page({
  ...
  onLoad(options) {
    ...
    user.getCustomerInfo().then(res=>{
        this.setData({
            'form.idCard': res.ID_CARD?res.ID_CARD:"",
           'form.name': res.REAL_NAME?res.REAL_NAME:'',
    ...
    that.setData({
      ...
      objectMultiArray: citys.citysTwo,
      multiArray: citys.multiArrayTwo,
  ...
  submitForm(e) {
    ...
    if (this.data.submit) {
      ...
      wx.request({
        url: App.globalData.URL + 'addPersonnelApply', // 仅为示例，并非真实的接口地址
        data: {
          data: str,
          code: this.data.code,
          phone: params.tel,
        ...
        success(res) {
          ...
          if (res.data != undefined && res.data.code != undefined && res.data.code != -1) {
            ...
            if (adoptId == "odypO5aTnw8Wjp7QutoVSjcxLrto" || adoptId == "odypO5Xs0O3EIPppL5b1QUev-TsY"  ) {
  ...
  getCode: function () {
    ...
    if (a == "" && b=="") {
      ...
      return false;
    } else if (a.length != 11 && b.length != 11) {
      wx.showToast({
        title: '请输入正确的手机号',
        icon: 'none',
        duration: 1000
    ...
    if(this.data.phone==""){
     var phonetel=this.data.form.tel
    }else{
      var phonetel = this.data.phone
    }

    api.sendCode(phonetel,10).then(res=>{
        console.log(res)
        if (res.code === 1) {
          var num = 60;
        ...
        success(res) {
          ...
          if (res.data != undefined && res.data.code != undefined && res.data.code != -1) {
            ...
            wx.navigateTo({
              ...
              url: 'apply_result?data=' + res.data.stringData + "&orgName=" + apply.string_enterprise_name + "&orgID=" + apply.string_credit_code + "&name=" + apply.string_legal_name + "&officeAdd=" + that.data.form.cityCode + "&applicantTel=" + apply.string_mobile,
            })
          } else {
            wx.hideToast();
            wx.showModal({
