...
Page({
  ...
  onLoad: function (options) {
    var that = this;
    var channelChanged = wx.getStorageSync("channelChanged"); 
    console.log(channelChanged,'是否选择渠道');
    if(channelChanged == true){
    var channelType = wx.getStorageSync("channelType");  
    var channelG = wx.getStorageSync("channelG");  
    console.log(channelType,'渠道的类型');
    console.log(channelG,'渠道G内容');
    that.setData({
      channelType: channelType,
      extraChannelInfo: channelG
    });
  }
      ...
      if (params.legalName && params.legalCard) {
        arr.push(
          Org.getEnterpriseInfo({
            ...
            companyCode: '',
            checkIdcard: params.legalCard,
            checkName: params.legalName,
          }).then((res) => {
            console.log('校验法人返回', res);
            log.info('校验法人返回', JSON.stringify(res));
  ...
  async createApply() {
    ...
    try {
      ...
      let isWhite1674 = JSON.parse(wdgl1674Result.resvFld1).whiteFlag;
