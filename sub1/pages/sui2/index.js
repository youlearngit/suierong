...
checkWeiManager(){
  let options = {
    url: 'sui/checkWeiManager.do',
    data: JSON.stringify({userid:this.data.empNo}),
  };
  return requestYT(options)
    .then((res) => {
      if (res.STATUS === '1' ) {
       let code = res.resultCode; //1 是 0 否
       if(code=='1'){
...
wx.switchTab({
  ...
  getApplyInfo() {
    ...
    // }
let url='/sub1/pages/sui2/index2';
  ...
  generateCardPoster() {
    ...
    let promise2 = new Promise(function (resolve, reject) {
      if (that.data.qrcodeUrl != '') {
        ...
        api.generateMiniCode('sub1/pages/sui2/index', that.data.empNo + 'a' + that.data.productCode).then((res) => {
          if (res) {
            img2 = res;
            that.setData({
              qrcodeUrl: img2,
  ...
  generateCardPoster4() {
    ...
    let promise3 = api
      .generateMiniCode('sub1/pages/sui2/index', that.data.empNo + 'a' + that.data.productCode)
      .then((res) => {
        if (res) {
          img3 = res;
          return res;
