...
Page({
  data: {
    ...
    posterImgs: [
      ...
      },
      {
        ...
        img: app.globalData.CDNURL + '/static/wechat/img/rec/activity_poster.png',
        title: '荐者有份',
        poster: '',
      },
      {
  ...
  onLoad: function (options) {
    console.log(options);
    let a1 = decodeURIComponent(options.afterEncode);
    // let a1 = decodeURIComponent('chNkoqn5/WrOa9vp7brMSLObVpldiDGY3bsgFQ3gbGubcsEoZp1GZ7on/eCKFh2Vc9tXlGkdlr0jLQ98cRLPbyVrFv1lkUHsIzx1g8U7tfAvg5CLHG6QkNC8Kn01W6LjHlVy6Zy7hK7L/22HfSf38IPJ8szSVDbLTDWur9IgCkA=');
    // let a = decodeURIComponent('qsg1p9ESX/sdciL9ZGkqy8eH7GFiKaoJ/0BvnUDRTKMwq9ED2O0c+kB52Sm/eufMhCSPdt+rvQIwda2chmByRU6jhDUN6LcMGtnHkHbLMkeRfO9u2Prtn7OBCmv3PeAiuNHsyp+8vTITUzhPvmWBjbyLeuuJts352AqNmgQehsk=');
    wx.setStorageSync('afterEncode', a1);
  ...
  onShow: function(){
    // 我的常州数据发送到后台start
    var that = this;
   let a = wx.getStorageSync('afterEncode');
    api.userInfoDecode(a).then((res) => {
      console.log(res,'解密');
      var resCom = res.completeDecode.replace(/[\r\n]/g,"").replace(/\ +/g,"");
      var realName = resCom.split(',')[0];
      var idNum = (resCom.split(','))[1];
      var phone = resCom.split(',')[2];
      console.log(idNum,'424');
      console.log(resCom,'resCom');
      // 获取小程序个人基本信息
      user.getCustomerInfo().then((res) => {
        console.log(res,'个人基本信息');

      // 校验个人信息是一致
      if(res.REAL_NAME && res.ID_CARD && res.TEL){
      if (res.REAL_NAME !== realName || res.ID_CARD !== idNum || res.TEL !== phone) {
        console.log(res.REAL_NAME,realName,res.ID_CARD,idNum,res.TEL,phone,'12345');
        wx.showModal({
          title: '提示',
        ...
        if( !res.TEL ){
          api.addPhone(phone).then((res)=>{
            console.log(res,'添加手机号');
            user.addCustomer(realName,idNum,phone,'0000','120026').then((res)=>{
              console.log(res,'渠道信息入库');
            });
         }).catch(
            wx.showModal({
              title: '提示',
              content: '您的身份信息已被注册,请前往认证',
              showCancel: false,
              confirmText: '确定',
              success: (result) => {
                if (result.confirm) {
...
chooseChannelType(e) {
  var that = this;
  console.log(e.detail,'渠道类型');
  this.setData({ channelType: e.detail});
  // wx.clearStorageSync();
  wx.removeStorageSync('channelType');
  wx.setStorageSync("channelType", e.detail);  
  if(e.detail){
    this.setData({channelChanged : true});
  }

},
getG(e){   //选择渠道其他
  console.log(e.detail.value,'渠道其他');
  wx.removeStorageSync('channelG');
  wx.setStorageSync("channelG", e.detail.value);  
},
  ...
  getAmount() {
    var that = this;
    wx.setStorageSync("channelChanged", that.data.channelChanged); 
  ...
  getApplyInfo() {
    ...
    Order.getUnfinishedOrder('/sub1/pages/changzhou/index');
  },
  ...
  generateCardPosterActivity() {
    ...
    let img3 = '';
    let promise1 = new Promise(function (resolve, reject) {
      let src = app.globalData.CDNURL + '/static/wechat/img/rec/activity_poster.png';
      wx.getImageInfo({
        src: src,
        success: function (res) {
          img1 = res.path;
