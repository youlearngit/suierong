...
import { getRegionCodeCz } from '../../../api/region';
...
Page({
  data: {
    ...
    canSubmitNode5: true,
    flag_0: true,
    flag_1: true,//1授 权 书(适用于个人经营贷及对公授信业务相关自然人)
    flag_2: true,//2江苏丹阳苏银村镇银行有限责任公司综合信息查询使用授权书（适用于自然人）
    flag_3: true,//3授 权 书(适用于法人及其他经济组织)
    flag_4: true,//4江苏丹阳苏银村镇银行有限责任公司综合信息查询授权委托书
    flag_5: true,
    //flag_6: false,
    //flag_7: true,
    isWhite: '1',
  ...
  async selectRegion(regionCode) {
    ...
    let arr = await getRegionCodeCz(regionCode);
    console.log('arr', arr);
    let city = arr[1].values.find((e) => e.adcode === `${enterpriseInfo.city}00`);
    let index = arr[1].values.indexOf(city);
    let t = ''
    t = arr[1].values[index];
    arr[1].values[index] = arr[1].values[0]
    arr[1].values[0] = t
    this.setData({
      multiArray3: arr,
      showRegionPicker: true,
  ...
  initOnloadData(options) {
    ...
    console.log("initOnloadData.options",options)
    var that = this;
    if (options.msg) {
      wx.showModal({
        title: '提示',
  ...
  initOnShowData() {
    ...
    user
      ...
      .then((res) => {
        let customerInfo = Object.assign(that.data.customerInfo, res);
        console.log('getIdentityInfo:', res);
        // that.setData({
        //   customerInfo,
  ...
  async chooseApplyType() {
    ...
    try {
      ...
      let List = [{showName: "个人经营贷（法人代表）", loanType: "20", showRemark: "贷款发放到企业法人代表个人账户，用于企业经营。"}]
      let isWhite = that.data.isWhite;
      console.log('wdgl1675list 1',List);
  ...
  async chooseApplyBody(e) {
    ...
    try {
      ...
      let applyBorr = '0';
      let channelAuthorizationType = '"20,30';
  ...
  async wdgl1679(cityCode, windFlag) {
    ...
    let listBusinessParams = [{
      "businessType":"0","city":"镇江市","cityChannel":"0000","loanType":"000","maxAmt":"200","termOpt":"12、24、36","termUnit":"月"
    }]
    // log.info('1679返回', JSON.stringify(res));

    // let listBusinessParams = '{"businessType":"0","city":"镇江市","cityChannel":"0000","loanType":"000","maxAmt":"200","termOpt":"12、24、36","termUnit":"月"}'
    // if (res.STATUS === '1' && res.resultCode === '0000' && res.listBusinessParams) {
      return listBusinessParams;
    // } else {
  ...
  async createApply() {
    ...
    try {
      ...
      let isWhite1674 = '0';//JSON.parse(wdgl1674Result.resvFld1).whiteFlag;
  ...
  readDocument(e) {
    ...
    if (e.detail.indexOf('99') > -1) {
      ...
      console.log(that.data.loanTpyeName)
      if (applyerType === '0') {
  ...
  measure() {
    ...
    return requestP({
      url: app.globalData.YTURL + 'sui/sui5001.do',
  ...
  chooseEnter2(companyName,applyerType,customerInfo,that){
    Org.getEnterpriseInfo({
      ...
      companyName,
        ...
        let arr = await getRegionCodeCz(enterpriseInfo.country);
        console.log(arr,'arr')
        ...
        if (country) {
          that.setData({
            ...
            enterWorkStation: arr[0].values[0].name + city.name + country.name,
          });
        } else {
          that.setData({
            multiArray3: arr,
